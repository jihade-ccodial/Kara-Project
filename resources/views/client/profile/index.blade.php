    ...
    <script>
        ...
        function formatProfileData (data) {
            ...
            @foreach($members as $m)
                if('{!! $m->id !!}' === data.id) {
                    $result = $(
                        '<div class="member-tbl-line"><div class="v-avatar member-avatar me-2" style="height: 28px; min-width: 28px; width: 28px; margin: 0; font-size: 14px"><span>{{ $m->firstName[0] . $m->lastName[0] }}</span></div>{{ $m->getFullNameAttribute() }}</div>'
                    );
                }
            @endforeach
